...
{
    let attrObj = col['type'].toString();
    let options = (col['type']['options']) ? col['type']['options'] : {};
    let values = col['type']['values'];
    // Sequelize.CHAR
    if (attrObj === "CHAR")
    {
        if (options.binary)
    ...
    if (attrObj === "DATE" || attrObj === "TIMESTAMP WITH TIME ZONE")
        return prefix + 'DATE';
...
{   
    ...
    if (defaultValue.toString() === "UUIDV1")
        return { internal: true, value: prefix + 'UUIDV1' };

    if (defaultValue.toString() === "UUIDV4")
        return { internal: true, value: prefix + 'UUIDV4' };
    
    if (typeof defaultValue.toString() === 'function')
...
const reverseModels = function(sequelize, models)
{
    ...
    for (let model in models)
    {
        ...
        if ((Object.keys(models[model].options)).includes('schema'))
        {   
            tables[models[model].tableName].options = {}
            tables[models[model].tableName].options.schema = 
                models[model].options.schema
        }
